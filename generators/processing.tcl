...
proc indent { lines depth } {
	set result {}
	set spaces [ gen::make_indent $depth ]

	foreach line $lines {
		set text [string trimleft $line]
		#Исключаем комментарии из транслитерации
...
proc p.print_to_file { fhandle functions header footer use_nogoto } {
    ...
    puts $fhandle \
        "// Generator adopted by Bardynin Dmitry, Astrakhan-Sochi, 2015-2020, ver.0.12"
...
proc rewire_wiring_if { gdb diagram_id } {
    ...
    while { 1 } {
        ...
        if { $new_text == "" } {
    	    graph::p.error $diagram_id [ list $item_id ] "If: Икона Вопроса должна быть заполнена."
    	  }
...
proc convert2msec { text } {
  ...
  # set halBirthMovie "00:10:10 "
	# set halBirthMovie [clock scan $halBirthMovie -format {%H:%M:%S}]
	# set halBirthMovie0 "00:00:00"
	# set halBirthMovie0 [clock scan $halBirthMovie0 -format {%H:%M:%S}]
	# puts $halBirthMovie
	# puts $halBirthMovie0
	# puts "!!> [expr { $halBirthMovie - $halBirthMovie0 } ] "
}
