...
#gen::add_generator Arduino.org gen_arduino::generate
...
set correct {
    ...
    tone noTone
    millis
    WiFi.mode WiFi.begin server.begin WiFi.config
    client.print client.flush client.stop
    Serial1.print Serial1.begin Serial1.println
    content.concat
}
...
proc generate { db gdb filename } {
    ...
    #tk_messageBox -icon info -message "Generating started"
    set gen::legalLoop "повторить(* {for (*} for(* while(* repeat(*"

    set gen_processing::prog_loop [ newline_cut [ join [ $gdb eval {
		select text
		from items
		where type = "beginend" AND ( text2 == "main")
	} ]]]

    set gen_processing::prog_setup [ newline_cut [ join [ $gdb eval {
        select text
        from items
        where type = "beginend" AND ( text2 == "setup")
    } ]]]
    if {$gen_processing::prog_loop == "" && $gen_processing::prog_setup == "" } {error "Отсутствует Пусковая диаграмма и Циклическая диаграмма. Укажите Пусковую или Циклическую диаграмму, щелкнув правой кнопкой по Началу соответствующей схемы. Можно указать обе диаграммы, при этом Пусковая выполнится однократно при запуске Ардуино, а Циклическая диаграмма будет выполняться многократно после Пусковой."}

    set gen_processing::prog_head [ newline_cut [ join [ $gdb eval {
        select text
        from items
        where type = "beginend" AND ( text2 == "header")
    } ]]]
    #if {$gen_processing::prog_head == "" } {error "Укажите Вступительную диаграмму, щелкнув правой кнопкой по Началу соответствующей схемы."}
    #tk_messageBox -icon info -message "prog_head!$gen_processing::prog_head!prog_setup!$gen_processing::prog_setup!prog_loop!$gen_processing::prog_loop"
    set names [ $gdb eval {
        SELECT name FROM diagrams
        } ]
    #$gen_processing::correct
    set newnames ""
    foreach name $names {
    ...
    #tk_messageBox -icon info -message "gen_processing::correct:$gen_processing::correct *" -title "names:$newnames*"
    append gen_processing::correct $newnames
    #tk_messageBox -icon info -message "gen_processing::correct:$gen_processing::correct *" -title "names:$newnames*"

    #log "started..."
    #log ">"
    #tk_messageBox -icon info -message "started"
...
#	exec start.exe $hfile
	...
	file mkdir $cut_tail
	file copy -force -- "[file dirname $hfile]\\data" $cut_tail
	file copy -force -- "$cut_tail.pde" $cut_tail
	file delete -force -- "$cut_tail.pde"
...
proc p.print_to_file { fhandle functions header footer use_nogoto } {
    ...
    #tk_messageBox -message "gen_processing::prog_loop!head!setup!$gen_processing::prog_loop!$gen_processing::prog_head!$gen_processing::prog_setup"
    ...
    if { $header != "" } {
        ...
        foreach function $functions {
            ...
            set type [ lindex $signature 0 ]
            if  {$type != "comment" } {
                if  {$name eq $gen_processing::prog_setup} {
                    set ready 1
                    puts $fhandle "\n\/\/ [mytranslit_declaration $name]"
        ...
        foreach function $functions {
            ...
            set type [ lindex $signature 0 ]
           	if  {$type != "comment" } {
                if  {$name eq $gen_processing::prog_loop} {
                    set ready 1
                    puts $fhandle "\n\/\/[ mytranslit_declaration $name]"
            		#set declaration [ build_declaration $name $signature ]
                    set declaration [ build_declaration "draw" $signature ]
            		set declaration [ mytranslit_declaration $declaration ]
            		puts $fhandle $declaration
            		set lines [ gen::indent $body 1 ]
            		append lines ";"
...
proc rewire_wiring_pause { gdb diagram_id } {
	  ...
	  while { 1 } {
        ...
        if { $s1 == 0 } {
	        ...
	        set new_text "[ convert2delay $text ] ; //pause $text"
        }
...
proc rewire_wiring_pause_ins { gdb diagram_id } {
    ...
    proc rewire_wiring_input_simple { gdb diagram_id } {
        ...
        while { 1 } {
            ...
            set new_text "$text = read(); // input_simple"
            $gdb eval {
            	update vertices
            	set text = :new_text
            	where vertex_id = :vertex_id
...
proc rewire_wiring_if { gdb diagram_id } {
    ...
    while { 1 } {
        ...
        if { [string first "\/\/" $text] == 0 } {
      		set new_text [string range $new_text [string first "\n" $new_text] end]
		    }
...
#SELECT name FROM diagrams WHERE name = :name
    set aaa $gen_processing::correct
    #append aaa " " $names
...
proc convert2delay { text } {
	if { [scan $text "%d"] == 0 } { return "delay( $text )" }
    set val [ digit_cut $text ]
	  if { $val == "миллисекунд" || $val == "мс" || $val == "мсек" || $val == "msec" || $val == "ms" } {
      return "delay([scan $text "%d"])"
    } elseif { $val == "секунд" || $val == "сек" || $val == "с" || $val == "sec" || $val == "s" || $val == "c" } {
      return "delay([expr [scan $text "%d"]*1000])"
    } elseif { $val == "минут" || $val == "мин" || $val == "м" || $val == "min" || $val == "m" } {
      return "delay([expr [scan $text "%d"]*60000 ])"
    } elseif {$val == "микросекунд" || $val == "microseconds" || $val == "мкс" || $val == "мксек" || $val == "mcsec" || $val == "mcs" } {
      return "//delayMicroseconds( [scan $text "%d"] )"
    } 
    return "delay( $text )"
}
