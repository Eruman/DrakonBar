...
#tk_messageBox -icon info -message "Generating started" -title mw::rem_visible
    ...
    set hfile [ replace_extension $filename "ino" ]
...
proc make_callbacks { } {
    ...
    set cbks [ dict replace $cbks signature gen_arduino::extract_signature ]
    set cbks [ dict replace $cbks body gen_arduino::generate_body ]
...
proc p.print_to_file { fhandle functions header footer use_nogoto } {
    ...
    if { $header != "" } {
        ...
        foreach function $functions {
            ...
            set type [ lindex $signature 0 ]
            if  {$type != "comment" } {
                if  {$name eq $gen_arduino::prog_setup} {
                    set ready 1
                    puts $fhandle "\n\/\/ $name"
        ...
        foreach function $functions {
            ...
            set type [ lindex $signature 0 ]
           	if  {$type != "comment" } {
                if  {$name eq $gen_arduino::prog_loop} {
                    set ready 1
                    puts $fhandle "\n\/\/ $name"
