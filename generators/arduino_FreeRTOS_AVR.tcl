...
gen::add_generator Arduino_FreeRTOS_AVR.org gen_arduino_FreeRTOS_AVR::generate

namespace eval gen_arduino_FreeRTOS_AVR {
...
#
# В RTOS все параметры предаются типа ( void *pvParameters )
#
	    	if { $name eq "setup" || $name eq "Настройка" } { 
			return "$returns $name\(\) \{"
		} else {
			return "$returns $name\( void *pvParameters \) \{"
		}
    	} else {
...
##################################################################
# FreeRTOS принимает только тип void"
##################################################################
#    		if { [lindex $a 1] == "" } {
    			set returns "void" 
#   		} else {
...
#    log "names:  $names \n \n \n" 
        ...
        set names [ $gdb eval {
    	    	 SELECT name FROM diagrams WHERE diagram_id > 2
        } ]
#	mw::my_message "names:  $names" 
#    log "names:  $names \n \n \n" 
...
#####################################
	puts $f "// Autogenerated for FreeRTOS"
	puts $f "\#include \<Arduino_FreeRTOS.h>"
	puts $f "\#include \<semphr.h>"
	puts $f "SemaphoreHandle_t xSerialSemaphore;"
foreach n $names {
        set n [ mytranslit_declaration $n ]
        set n [ my_name_translit $n ]
    	puts $f "void $n\( void *pvParameters );" 
}
	puts $f "\n"
#####################################
...
proc make_callbacks { } {
    ...
    set cbks [ dict replace $cbks signature gen_arduino_FreeRTOS_AVR::extract_signature ]
    set cbks [ dict replace $cbks body gen_arduino_FreeRTOS_AVR::generate_body ]
...
proc p.print_to_file { fhandle functions header footer use_nogoto } {
    ...
    puts $fhandle \
        "// Generator adopted by Bardynin Dmitry, Astrakhan, 2015, ver.0.01"
...
############################
            		puts $fhandle "\nvoid loop() \{"
            		puts $fhandle "  // Empty. Things are done in Tasks."
            		puts $fhandle "\}"
############################
            	   }
            }
...
#        }
        ...
        foreach function $functions {
            ...
            if  {$type != "comment"   
                         ...
                         &&  $name !="_Программа(исключено)"  && $name!="Настройка"	} {
            		puts $fhandle ""
#############################
...
#################################################
# пустой цикл нужен для организации бесконечных тасков
№
#################################################
        } else {
...
#            set new_text "delay($text); //pause"
            set new_text "vTaskDelay($text); //each one tick delay (15ms) in between reads for stability"
...
# Нужно поправить очередность переработки имени, отделив имя от параметра в скобках
#########################
            set exper [string last ")" $text2]
            if {$exper < 0} { set new_text "$text = $text2\(); // input without parameter" 
                } else { set new_text "$text = $text2; // input with parameter. NEED EDIT!!!" } 
...
proc rewire_wiring_timer { gdb diagram_id } {
    ...
    log "rewire_timer: $diagram_id"
        set starts [ $gdb eval {
            ...
            set new_text "$text; // Set timer $text"
            $gdb eval {
            	update vertices
            	set text = :new_text
            	where vertex_id = :vertex_id
