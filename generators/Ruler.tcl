# Ruler-generator by Bardynin Dmitry aka Eruman
# Astrakhan-Sochi 2015-2019

gen::add_generator Верификатор gen_Ruler::generate

namespace eval gen_Ruler {
...
proc generate { db gdb filename } {
    ...
    #tk_messageBox -icon info -message "gen_Ruler::correct:$gen_Ruler::correct *" -title "names:$newnames*"
    append gen_Ruler::correct $newnames
    #tk_messageBox -icon info -message "gen_Ruler::correct:$gen_Ruler::correct *" -title "names:$newnames*"
    ...
    set diagrams [ $gdb eval {
    	...
    	if { [ graph::errors_occured ] } { return }
            set hfile [ replace_extension $filename "rul" ]
...
#	exec start.exe $hfile
	after 1000
	set command "[auto_execok start] {} [list $hfile] FNV32"
#	tk_messageBox -icon info -message "[auto_execok start] {} [list $hfile] FNV32" -title ":)"
	if { $command == {} } { return }
...
proc make_callbacks { } {
    ...
    set cbks [ dict replace $cbks signature gen_Ruler::extract_signature ]
    set cbks [ dict replace $cbks body gen_Ruler::generate_body ]
...
proc is_dia_name { name gdb} {
    ...
    set name [string trimleft $name]
    #tk_messageBox -icon info -message $name -title "$name"
    foreach names $gen_Ruler::correct {
