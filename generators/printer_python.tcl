...
proc p.generate { db gdb filename pversion } {
    ...
    set gen_pripy::prog_loop [ newline_cut [ join [ $gdb eval {
		select text
		from items
		where type = "beginend" AND ( text2 == "main")
	  } ]]]

    set gen_pripy::prog_setup [ newline_cut [ join [ $gdb eval {
        select text
        from items
        where type = "beginend" AND ( text2 == "setup")
    } ]]]
    
    #tk_messageBox -message $gen_pripy::prog_loop
    #tk_messageBox -message $gen_pripy::prog_setup
    
    if {$gen_pripy::prog_setup == "" } {
		tk_messageBox -message "Отсутствует Пусковая диаграмма. \
		Укажите Пусковую диаграмму, щелкнув правой кнопкой по Началу соответствующей схемы." 
		return 
		}

    set gen_pripy::prog_head [ newline_cut [ join [ $gdb eval {
        select text
        from items
        where type = "beginend" AND ( text2 == "header")
    } ]]]
    if {$gen_pripy::prog_head == "" } { 
		tk_messageBox -message "Укажите Вступительную диаграмму, щелкнув правой кнопкой по Началу соответствующей схемы."
		#error "Укажите Вступительную диаграмму, щелкнув правой кнопкой по Началу соответствующей схемы."
		return	
		}
    set names [ $gdb eval {
    ...
    append gen_pripy::correct $newnames
    
    #tk_messageBox -message $gen_pripy::correct
...
proc p.print_to_file { fhandle procs class methods header footer } {
    ...
    puts $fhandle \
        "### Generator adopted by Bardynin Dmitry, Astrakhan-Sochi-Sirius, 2023, ver.0.12"
	...
	foreach function $procs {
		unpack $function diagram_id name signature body
	set name [ newline_cut $name ]
	set type [ lindex $signature 0 ]
	
	if  {$type != "comment" } {
		if  {$name eq $gen_pripy::prog_head} {
			puts $fhandle "\n\# $name"
			set lines [ indent $body 0 ]
			puts $fhandle $lines
			puts $fhandle ""
		} else {
			puts $fhandle "\n\# $name"
    ...
    puts $fhandle ""
    puts $fhandle "#Стартуем процедуру, назначенную стартовой"
    puts $fhandle "[my_name_translit $gen_pripy::prog_setup](\)"
