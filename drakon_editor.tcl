...
#tk_messageBox -icon error -message "Hello!!"
...
variable db mdb
################################################################################
proc speak {word {param ""} args} {
	set t "Govorilka//Govorilka_cp.exe $param $args \"$word\""
	catch {eval exec $t} err
}
set myhelp ""
set myhelpCounter 0
set my_filename ""
...
proc createfile { filename } {
	...
	#mwc::do_create_dia "Программа" 1 0 drakon
	#mwc::do_create_dia "Настройка" 2 0 drakon
	#mwc::do_create_dia "Заголовок" 2 0 drakon
	return 1
}
