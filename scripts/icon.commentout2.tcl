...
proc commentout2.switch { } {
	return [ mc2 "Flip horizontally" ]
}

proc commentout2.create { item_id diagram_id x y } {
...
proc commentout2.fit { tw th tw2 th2 x y w h a b } {

	return [ action.fit $tw $th 0 0 $x $y $w $h $a $b ]
}


proc commentout2.lines { x y w h a b } {
	if { $b == 1 } {
		set left [ expr { $x + $w } ]
...
proc commentout2.box { x y w h a b } {
	if { $b == 0 } {
		set left [ expr { $x - $w - $a } ]
		set right [ expr { $x + $w } ]
	} else {
...
proc commentout2.icons { text text2 color x y w h a b } {
...
proc commentout2.handles { x y w h a b } {
...
proc commentout2.branch_handle { dx dy x y w h a b } {
	if { $b == 0 } {
		set a2 [ expr $a - $dx ]
	} else {
		set a2 [ expr $a + $dx ]
...
proc commentout2.nw { dx dy x y w h a b } {
	return [ action.nw $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.n { dx dy x y w h a b } {
	return [ action.n $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.ne { dx dy x y w h a b } {
	return [ action.ne $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.e { dx dy x y w h a b } {
	return [ action.e $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.sw { dx dy x y w h a b } {
	return [ action.sw $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.s { dx dy x y w h a b } {
	return [ action.s $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.se { dx dy x y w h a b } {
	return [ action.se $dx $dy $x $y $w $h $a $b ]
}

proc commentout2.w { dx dy x y w h a b } {
