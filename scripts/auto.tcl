...
############################################################################
# Проверка иконок на вертикальность auto.tcl
proc p.check_icons_on_verticals { diagram_id  starts } {
	...
	gdb eval {
		...
		if { $left != "" && $type != "insertion" && $type != "action"} {
			# tk_messageBox -message "diagram_id = $diagram_id type $type "
			p.unexpected_edge $left
		}
		# if { $left != "" && $type == "insertion"} {
		# 	tk_messageBox -message "diagram_id = $diagram_id type $type "
		# 	tk_messageBox -message "left $left vertex_id $vertex_id item_id $item_id"
		# }
		if { $type != "if" && $type != "pause" && $right != "" } {
			# tk_messageBox -message "diagram_id = $diagram_id type $type "
			p.unexpected_edge $right
		}
		# if { $type != "if" && $type == "pause" && $right != "" } {
		# 	tk_messageBox -message "diagram_id = $diagram_id type $type "
		# 	tk_messageBox -message "p.check_right_goes_up $item_id [ p.check_right_goes_up $item_id ] "
		# 	tk_messageBox -message "right $right "
		# }
		if { ($up == "" || $down == "") && $type != "pause"  && $type != "insertion" && $type != "action" } {
			# tk_messageBox -message "diagram_id = $diagram_id type $type "
