...
proc beginend.icons { text text2 color x y w h a b } {
	...
	set x3 [ expr { $x + $w * 0.9 } ]
	set y3 [ expr { $y - $h * 1.5 } ]
	set coords3 [ list $x2 $y2 ]
	if {[expr {![catch {file lstat "$filepath/$text2.png" finfo}]} ] == 1} {
		set text_prim2 [ create_text $x $y3 "" $tc secondary]
		set figack [ make_prim figack image $coords3 "$filepath/$text2.png" $fg $bg $cdbox ]
	} else {
		set text_prim2 [ create_text $x $y3 "\[[ mc2 $text2]\]" "#33aa20" secondary]
		set figack [ make_prim figack image $coords3 "$filepath/$text.png" $fg $bg $cdbox ]
	}
	if {$text2 !="" } {
		return [ list $figack $rect $text_prim2 $text_prim ]
	} else {
