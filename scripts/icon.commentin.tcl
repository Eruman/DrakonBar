...
proc commentin.icons { text text2 color x y w h a b } {
	...
	set coords3 [ list $left $top [expr { $left - 10 }] [expr { $top - 10 }]	]
	#set myimage [image create photo -file ./images/drakosha3.ico]
	#set coords3 [ list [ expr { $x - $w } ] [ expr { $y - $h } ] ]
	set coords3 [ list $x $y ]
	#set figack [ make_prim figack image $coords3 "./images/drakosha3.ico" $fg $bg $cdbox ]
	set figack [ make_prim figack image $coords3 $text2 $fg $bg $cdbox ]
	#return [ list $back $rect $text_prim $text_prim2 $figack]
	return [ list $rect1 $screen $text_prim $figack]
