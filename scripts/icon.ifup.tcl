...
proc ifup.create { item_id diagram_id x y } {
...
proc ifup.fit { tw th tw2 th2 x y w h a b } {
...
proc ifup.icons { text text2 color x y w h a b } {
...
proc ifup.branch_handle { dx dy x y w h a b } {
	set a2 [ expr $a + $dx ]
	if { $a2 < 20 } { set a2 20 }
	set y2 [ expr { $y + $dy } ]
...
proc ifup.nw { dx dy x y w h a b } {
	return [ action.nw $dx $dy $x $y $w $h $a $b ]
}

proc ifup.n { dx dy x y w h a b } {
	return [ action.n $dx $dy $x $y $w $h $a $b ]
}

proc ifup.ne { dx dy x y w h a b } {
	return [ action.ne $dx $dy $x $y $w $h $a $b ]
}

proc ifup.e { dx dy x y w h a b } {
	return [ action.e $dx $dy $x $y $w $h $a $b ]
}

proc ifup.sw { dx dy x y w h a b } {
	return [ action.sw $dx $dy $x $y $w $h $a $b ]
}

proc ifup.s { dx dy x y w h a b } {
	return [ action.s $dx $dy $x $y $w $h $a $b ]
}

proc ifup.se { dx dy x y w h a b } {
	return [ action.se $dx $dy $x $y $w $h $a $b ]
}

proc ifup.w { dx dy x y w h a b } {
