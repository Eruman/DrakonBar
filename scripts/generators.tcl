...
proc download { } {
	...
	mw::set_status "Downloading code for '$language' language..."
	update
	...
	if { ![ graph::errors_occured ] } {
		if { [ catch { p.do_download $generator $language } result ] } {
			if { ![ graph::errors_occured ] } {
				mw::show_red_result
...
proc generate { } {
	...
	mw::set_status "Verify all..."
	graph::verify_all $db
	mw::set_status "Generating..."
...
proc p.rewire_loop { gdb start callbacks append_semicolon } {
	...
	#####################################################################
	set i 0
	foreach j $gen::legalLoop {
		#tk_messageBox -icon info -message $j -title "$text"
		if { [ string match $j $text ]} {
			set type "anyloop"
			set parts $text
		}
		incr i
	...
	if { $type == "for" } {
	#	tk_messageBox -icon info -message "$gdb \n$start \n$end \n$parts \n$append_semicolon" -title "$parts"
		p.rewire_for $gdb $start $end $parts $append_semicolon
	} elseif { $type == "foreach" } {
		p.rewire_foreach $gdb $diagram_id $start $end $parts $callbacks
	} else {
	############################################################################
		p.rewire_anyloop $gdb $start $end $parts $append_semicolon
	}
...
##################################################################
proc p.rewire_anyloop { gdb start end parts append_semicolon  } {
	set diagram_id [ p.vertex_diagram $gdb $start ]

	set line_print [ $gdb eval { select vertex_id,type,text from vertices where diagram_id = :diagram_id} ]
	#log $line_print
	set link_print [ $gdb eval { select *	from links  }]
	#log "---------"
	#log $link_print
	#log "===================="

	set init "$parts\{ \/\/ block_start"
	set check "true"
	set advance "\} \/\/ block_finish"
	set item_id [ p.vertex_item $gdb $start ]
#tk_messageBox -icon info -message $item_id
	set init_id [ p.insert_vertex $gdb $diagram_id [ append_digits $item_id 0001 ] "action" $init "" 0 ]
	#set check_id [ p.insert_vertex $gdb $diagram_id [ append_digits $item_id 0002 ] "if" $check "" 0 ]
	set advance_id [ p.insert_vertex $gdb $diagram_id [ append_digits $item_id 0003 ] "action" $advance "" 0 ]
##log "item_id^$item_id = start_vertex^$start \ninit_id^$init_id \ncheck_id^$check_id \nadvance_id^$advance_id \nend^[ p.vertex_item $gdb $end ] = end_vertex^$end"
#tk_messageBox -icon info -message $init_id
#tk_messageBox -icon info -message $check_id
#tk_messageBox -icon info -message $advance_id
	###proc p.relink { gdb old_dst new_dst }
	p.relink $gdb $end $advance_id
	###proc p.link { gdb src ordinal dst }
	#p.link $gdb $advance_id 1 $check_id
	#p.link $gdb $advance_id 1 $init_id
	p.relink $gdb $start $init_id
	#p.link $gdb $init_id 1 $check_id
	#p.link $gdb $init_id 1 $advance_id
	set first_loop [ p.get_next $gdb $start 2 ]
	set after_loop [ p.get_next $gdb $start 1 ]
#tk_messageBox -icon info -message $first_loop
#tk_messageBox -icon info -message $after_loop
	p.link $gdb $advance_id 1 $after_loop
	p.link $gdb $init_id 2 $first_loop
	$gdb eval {
		delete from links where src = :start;
		delete from vertices where vertex_id in (:start, :end);
	}
...
proc print_node { texts node callback depth } {
	...
	#set if_close [ get_callback $callback if_close ]
